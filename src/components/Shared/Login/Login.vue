<template>
  ...
  <div @keypress.enter="signin">
    ...
    <div class="login">
      ...
      <input type="button" value="SignUP" @click="gotosignup" />
            <input type="button" value="Go to YourWear" @click="gotoyourwear" />

      <h1 v-show="showwarn" class="warning">{{ warn }}</h1>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    signin() {
      ...
      var user={username:this.user,email:this.user,password:this.pass}
       this.$store.dispatch('Login', {data:JSON.stringify(user)}).then(res=>{
        console.log(res)
        if(res.status==200)
      ...
      gotoyourwear(){this.$router.push({name:'Home'})}
